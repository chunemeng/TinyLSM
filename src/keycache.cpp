...
namespace LSMKV {
	...
	struct LoserTree {
		...
		//		void Adjust(size_t index, size_t end = 0) {
		//			size_t parent = index + n_;
		//			Table::TableIterator* s;
		//			Table::TableIterator* l;
		//
		//			size_t * ptree = loser_;
		//			while ((parent /= 2) > end) {
		//				size_t & tparent = ptree[parent];
		//				switch (cmp(s = m_way_[tparent].get(), l = m_way_[index].get())) {
		//				case 0:
		//					[[unlikely]]
		//					if (index < tparent) {
		//						tparent = index;
		...
		std::pair<size_t, Table::TableIterator*> increment() {
			size_t top = loser_[0];
			auto iter = m_way_[top].get();
			iter->merge_next();
			Adjust(top);
			return { top, iter };
		}

		//		switch (cmp(left_iter, right_iter)) {
		//		case 0:
		//			[[unlikely]]
		//			if (left < right) {
		//				winner = left;
		//				loser = right;
		//				LoserTree::ajust(right_iter);
		//			}
		//			else {
		//				winner = right;
		//				loser = left;
		//				LoserTree::ajust(left_iter);
		//			}
		//			break;
		//		case 1:
		//			loser = left;
	...
	void KeyCache::Merge(uint64_t file_no,
		...
		if (n == 2) [[unlikely]] {
			...
			uint64_t timestamp{};
			int i;
			std::tie(index, iter) = loser->increment();
			Slice value_slice;
			do {
				table_cache = new Table(file_no++);
				char* tmp = table_cache->reserve(16384);
				for (i = 0; i < 408; std::tie(index, iter) = loser->increment()) {
					if (iter->timestamp() == 0) [[unlikely]] {
						break;
					}

					if (timestamp != 0 && (wait_insert_key == iter->key())
						&& (timestamp > iter->timestamp() || (timestamp == iter->timestamp() && old_index < index))) {
						continue;
					}
